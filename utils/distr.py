...
class Distr:
    ...
    def p_event_more_eq_than_x(self, x):
    ...
    def p_event_less_eq_than_x(self, x):
        num_elements_less_eq_than_x = 0
        for element in self.sample:
            if element <= x:
                num_elements_less_eq_than_x += 1
        return num_elements_less_eq_than_x/len(self.sample)
